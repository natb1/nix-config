...
| Fate of the Drive/Photos/GCS/MacBook/Flickr copies (iCloud has its own gated trim, [Step 5](#step-5--clear-icloud-storage-automatically)) | After the restore test and the [retirement gate](#before-any-original-is-retired) | Keep, or retire in favour of `/srv/media` + Hetzner. Not before both, either way |
...
| `desk` gnome-keyring password | [The desktop session](#the-desktop-session) | The one that matters most on `desk`. With autologin, no screen lock and an unencrypted disk, it is the only at-rest protection for Chrome's saved passwords (including the Hetzner account password, for now), the rclone config password and the Apple ID password. The keyring file under `~/.local/share/keyrings/` is on that disk, so this password is what an offline attacker faces: a long passphrase used nowhere else, typed once per boot. Memorized, plus the vault and the paper. Once Chrome's passwords move to the vault, the keyring holds only machine tokens that can be recreated in minutes, and it matters much less. Changing it (Seahorse) re-encrypts the file in place, and nothing else holds a copy |
| Samba password (`n8`) | [Step 1](#step-1--srvmedia-as-a-network-share) | Opens `/srv/media` read-write, deletion included, so the damage it can do is to the media itself, and the append-only backup is what limits that. It is held in three places: `desk`'s passdb (`/var/lib/samba/private/`, root-only), `mba`'s login keychain and the iPhone's Files app. The passdb holds an unsalted NT hash, which is quick to crack from the unencrypted disk, and the hash alone is enough to log in to SMB. So it gets 20+ random characters, is unique, lives in the vault and is pasted, never typed. Traffic stays inside the tailnet, so the network is not the weak point. Rotation: `sudo smbpasswd n8`, then update the `desk` entry in `mba`'s Keychain Access and re-enter it in Files on the iPhone. Not part of the recovery plan: without `desk`, the way back to the media is restic, not SMB |
| `mba` admin password and login keychain | Not yet in this plan | Assuming the admin account is also the everyday login (if it is a separate account, its password is vault-only and the login user's takes the memorized slot). One password with three jobs: the macOS login, `sudo` for `darwin-rebuild`, and, by default, the login keychain's password. The keychain holds the Samba password and whatever Safari and other apps saved. `mba` is also where the vault gets unlocked, so if `mba` is compromised, an unlocked vault is too. Memorized, unique, plus the vault and the paper. **Check and record:** FileVault is on (`fdesetup status`), and its recovery key is in the vault and on paper, not only escrowed to the Apple ID. The keychain password still matches the login password: they drift apart if the login password is *reset* (from recovery, or via the Apple ID) instead of *changed* in System Settings, and a reset starts a new, empty keychain. Optional: Touch ID for `sudo` through nix-darwin's `security.pam.services.sudo_local.touchIdAuth`, so the password is typed, and seen, less often |
