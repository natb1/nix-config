...
| Credentials, hardening and recovery | [Before any original is retired](#before-any-original-is-retired) | **Kept out of this public repo** since 2026-09-25, in the owner's private notes: which credential is kept where, what each one unlocks, the Storage Box hardening status and the written recovery plan. That pairing is a map for an attacker. This plan keeps only what the machines need, the paths and modes of hand-provisioned files, and the gate below |
...
- [ ] **Credential, hardening and recovery items closed.** Tracked
      privately (see the open decisions). In short: the restic repository
      password and the prune key exist somewhere other than `desk`, and someone
      other than you could restore the media
...
| 4′. Music | `media-stage group staging/<batch>`, then `beet stage-review staging/<batch>` | `group` gives each album its own folder (the `audio` drive was one flat folder of 800 tracks). `stage-review` ([`beetsplug/stagereview.py`](../pkgs/media-stage/beetsplug/stagereview.py)) is beets with no prompts: albums with a strong MusicBrainz match are imported; the rest — weak or no match, or a duplicate of an album already filed — are written with their top five candidates to `staging/<batch>.review.json` and stay in staging |
...
Steps 1–5 and 6–7 are Claude's to run. Step 5′ is the one place the procedure waits for a person, and nothing in the batch is applied before it. beets 2.x needs `musicbrainz` in its plugin list to match anything at all ([`hosts/desk/home/media.nix`](../hosts/desk/home/media.nix)).
